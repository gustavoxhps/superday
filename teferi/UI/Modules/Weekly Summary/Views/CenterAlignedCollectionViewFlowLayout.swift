...
public class CenterAlignedCollectionViewFlowLayout: UICollectionViewFlowLayout
{
    ...
    {
        guard let oldAttributes = super.layoutAttributesForElements(in: rect) else { return nil }
        let attributes = oldAttributes.map {
            $0.copy() as! UICollectionViewLayoutAttributes
        }
        
        // We will do centering alignment only on the Cell layout attributes
        let cellAttributes = attributes.filter { layout in
